-- SecuredLanding v4.2.0 - NMI approved-payment investment finalization repair
...
create table if not exists public.nmi_payment_transactions (
  ...
  nmi_transaction_id text not null,
  transaction_kind text not null default 'investment',
...
begin
  ...
  -- Every successful investment increases invested balance, including pure-card investments.
  insert into public.investor_wallets(user_id,available_balance,invested_balance,updated_at)
  values(v_user_id,0,p_total_amount,now())
  on conflict (user_id) do update
    set invested_balance=coalesce(public.investor_wallets.invested_balance,0)+excluded.invested_balance,
        updated_at=now();
