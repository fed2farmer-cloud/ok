...
export default function InvestmentCertificateDetails() {
  ...
  const [investment, setInvestment] =
    useState<CertificateInvestment | null>(null);

  const [publicLoanNumber, setPublicLoanNumber] =
  ...
  useEffect(() => {
    async function load() {
      ...
      const certificate = data as CertificateInvestment;

      const isOwner =
        certificate.current_owner_id === auth.user.id ||
        (!certificate.current_owner_id &&
          certificate.investor_id === auth.user.id);
      ...
      setInvestment(certificate);      if (certificate.loan_application_id) {
        const {
          ...
          .select("loan_number")
          .eq("id", certificate.loan_application_id)
          .maybeSingle();

        if (loanError) {
          console.error(
            "Unable to load public loan number:",
            loanError.message
          );
        } else if (loanApplication?.loan_number) {
          setPublicLoanNumber(
    ...
    return (
      <AppLayout>
        <div className="mx-auto max-w-3xl px-4 py-16">
          <div className="rounded-2xl border border-rose-800 bg-rose-950/40 p-6 text-rose-200">
      ...
      <main className="mx-auto max-w-5xl px-4 py-10 text-white print:bg-white print:text-black">
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
          ...
          <button
            ...
            className="rounded-xl bg-amber-400 px-5 py-2 text-sm font-black text-slate-950 hover:bg-amber-300"
          >
            Print Certificate
          </button>
        </div>
        ...
        <section className="relative overflow-hidden rounded-[2rem] border border-amber-400/50 bg-gradient-to-br from-slate-950 via-emerald-950 to-slate-950 p-7 shadow-2xl sm:p-12 print:border-2 print:border-black print:bg-white">
          ...
          <div className="relative text-center">
            <p className="text-xs font-black uppercase tracking-[0.32em] text-amber-300 print:text-black">
              SecuredLanding
            </p>
            ...
            <p className="mt-3 text-sm text-slate-300 print:text-black">
              Permanent record of an ownership interest
              in a land-backed loan
            </p>
          </div>
          ...
          <div className="relative mt-10 rounded-2xl border border-white/10 bg-white/5 p-5 text-center print:border-black print:bg-white">
            <p className="text-xs font-bold uppercase tracking-widest text-slate-400 print:text-black">
              Certificate Number
            </p>

            <p className="mt-2 break-all font-mono text-xl font-black text-amber-300 sm:text-3xl print:text-black">
            ...
            {[
              ...
              [
                "Issued",
                investment.certificate_issued_at
                  ? new Date(
                      investment.certificate_issued_at
                    ).toLocaleDateString()
                  : "N/A",
              ...
              <div
                key={label}
                className="rounded-2xl border border-white/10 bg-black/20 p-4 print:border-black print:bg-white"
              >
          ...
          <div className="relative mt-8 grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4 print:border-black print:bg-white">
              ...
              <p className="mt-1 text-xs text-slate-400 print:text-black">
                Recorded transfers: {investment.transfer_count || 0}
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="mt-5 space-y-4">
            {history.length === 0 ? (
              <p className="text-slate-400 print:text-black">
                No ownership events found.
              </p>
            ) : (
              history.map((event) => (
                <div
                  key={event.id}
                  className="rounded-2xl border border-slate-800 p-4 print:border-black"
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="font-black">
                        {statusLabel(event.transfer_type)}
                      </p>
                    ...
                    <span className="rounded-full bg-emerald-950 px-3 py-1 text-xs font-bold text-emerald-300 print:border print:border-black print:bg-white print:text-black">
                      {statusLabel(event.transfer_status)}
                    </span>
                  </div>
                  ...
                  <div className="mt-3 space-y-1 text-sm">
                    <p>
                      Principal:{" "}
                      <strong>
                        {money(event.principal_transferred)}
                      </strong>
                    </p>
