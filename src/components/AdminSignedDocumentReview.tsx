    ...
    <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-black text-slate-950">Closing document status and review</h2>
          <p className="mt-1 text-sm text-slate-600">See every generated closing document, including items still awaiting borrower upload, and review signed copies before funding.</p>
        </div>
      ...
      {loading ? <p className="mt-4 text-sm text-slate-500">Loading signed documents…</p> : rows.length === 0 ? (
        <p className="mt-4 rounded-xl bg-slate-50 p-4 text-sm font-semibold text-slate-600">No closing documents have been generated yet.</p>
      ) : (
            ...
            return (
              <article key={row.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                {row.signed_storage_path ? (
                  ...
                  <p className="mt-4 rounded-xl border border-dashed border-slate-300 bg-white p-3 text-sm font-semibold text-slate-500">Awaiting borrower signed upload.</p>
                )}
              </article>
            );
          })}
